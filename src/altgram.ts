/** AltGram Bot API client for Larpp bot. */

const ALTGRAM_API_URL = process.env.ALTGRAM_API_URL || 'http://188.134.95.254:2610'
const BOT_TOKEN = process.env.BOT_TOKEN || ''

export type TgEntity = { type: string; offset: number; length: number; url?: string }
export type TgInlineKeyboardMarkup = { inline_keyboard: { text: string; callback_data?: string; url?: string }[][] }
...
async function tgFetch<T>(method: string, body: Record<string, unknown>) {
  ...
  if (!data.ok) console.error(`[altgram] ${method} failed:`, data.error_code, data.description)
  return data
}
...
export const altgram = {
  ...
  async sendMessage(params: {
    chat_id: number | string; text: string; entities?: TgEntity[]
    reply_markup?: TgInlineKeyboardMarkup; reply_to_message_id?: number; disable_web_page_preview?: boolean
  }) {
  ...
  async sendPhoto(params: {
    chat_id: number | string; photo: string; caption?: string; reply_markup?: TgInlineKeyboardMarkup
  }) {
  ...
  async deleteWebhook() { return tgFetch<boolean>('deleteWebhook', {}) },
  ...
  async getAvailableGifts() {
    return tgFetch<{ count: number; gifts: Array<{ id: string; star_count: number; remaining_count?: number }> }>('getAvailableGifts', {})
  },
}
